...
class Command(BaseCommand):
    ...
    help = "Resuelve enfrentamientos de liga"
    ...
    def handle(self, *args, **kwargs):
        ...
        enfrentamientos = (
            Enfrentamiento.objects.all()
        )

        for enfrentamiento in enfrentamientos:

            af_local = enfrentamiento.af_local
            af_visitante = enfrentamiento.af_visitante

            if af_local > af_visitante:

                enfrentamiento.resultado = "LOCAL"
                enfrentamiento.puntos_local = 3
                enfrentamiento.puntos_visitante = 0

            elif af_local < af_visitante:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Enfrentamientos resueltos: {actualizados}"
            )
        )